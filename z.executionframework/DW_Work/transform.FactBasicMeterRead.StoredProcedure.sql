...
GO
CREATE PROCEDURE [transform].[FactBasicMeterRead]
...
--/
	...
	INSERT INTO temp.FactBasicMeterRead (
	  ...
	  FROM /* Staging */ DW_Staging.orion.utl_meter_read INNER JOIN /* Staging */ DW_Staging.orion.utl_read_type ON utl_read_type.read_type_id = utl_meter_read.read_type_id INNER JOIN /* Staging */ DW_Staging.orion.utl_read_sub_type ON utl_read_sub_type.read_sub_type_id = utl_meter_read.read_sub_type_id INNER JOIN /* Staging */ DW_Staging.orion.nc_product_item ON nc_product_item.seq_product_item_id = utl_meter_read.seq_product_item_id INNER JOIN /* Staging */ DW_Staging.orion.nc_product ON nc_product.seq_product_id = nc_product_item.seq_product_id LEFT JOIN /* Dimensional */ DW_Dimensional.DW.DimAccount AS _DimAccount ON _DimAccount.AccountKey = nc_product.seq_party_id AND _DimAccount.Meta_IsCurrent = 1 LEFT JOIN /* Dimensional */ DW_Dimensional.DW.DimService AS _DimService ON _DimService.ServiceKey = CAST(nc_product_item.site_id AS int) AND _DimService.Meta_IsCurrent = 1 LEFT JOIN productKey AS _productKey ON _productKey.seq_product_item_id = nc_product_item.seq_product_item_id LEFT JOIN /* Dimensional */ DW_Dimensional.DW.DimProduct AS _DimProduct ON _DimProduct.ProductKey = _productKey.ProductKey AND _DimProduct.Meta_IsCurrent = 1 INNER JOIN /* Dimensional */ DW_Dimensional.DW.DimMeterRegister AS _DimMeterRegister ON _DimMeterRegister.MeterRegisterKey = utl_meter_read.meter_id AND _DimMeterRegister.Meta_IsCurrent = 1 WHERE utl_meter_read.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR utl_read_type.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR utl_read_sub_type.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID;
