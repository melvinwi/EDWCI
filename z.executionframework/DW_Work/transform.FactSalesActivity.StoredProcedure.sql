...
CREATE PROCEDURE [transform].[FactSalesActivity]
...
--/
	...
	INSERT INTO temp.FactSalesActivity (
	  ...
	  FROM DW_Staging.orion.nc_sales_involvement INNER JOIN DW_Staging.orion.nc_involvement_type ON nc_involvement_type.seq_involve_type_id = nc_sales_involvement.seq_involve_type_id LEFT JOIN DW_Staging.orion.nc_product ON nc_product.seq_product_id = nc_sales_involvement.seq_product_id LEFT JOIN DW_Dimensional.DW.DimAccount AS _DimAccount ON _DimAccount.AccountKey = nc_product.seq_party_id AND _DimAccount.Meta_IsCurrent = 1 LEFT JOIN DW_Staging.orion.nc_product_item ON nc_product_item.seq_product_item_id = nc_sales_involvement.seq_product_item_id LEFT JOIN DW_Dimensional.DW.DimService AS _DimService ON _DimService.ServiceKey = CAST(nc_product_item.site_id AS int) AND _DimService.Meta_IsCurrent = 1 LEFT JOIN productKey AS _productKey ON _productKey.seq_product_item_id = nc_product_item.seq_product_item_id LEFT JOIN DW_Dimensional.DW.DimProduct AS _DimProduct ON _DimProduct.ProductKey = _productKey.ProductKey AND _DimProduct.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimPricePlan AS _DimPricePlan ON _DimPricePlan.PricePlanKey = N'DAILY.' + CAST(_productKey.price_plan_id AS nvarchar(20)) AND _DimPricePlan.Meta_IsCurrent = 1 LEFT JOIN crmElementHierarchy AS _crmElementHierarchy ON _crmElementHierarchy.element_id = nc_sales_involvement.seq_party_id AND _crmElementHierarchy.recency = 1 LEFT JOIN DW_Dimensional.DW.DimRepresentative AS _DimRepresentative ON _DimRepresentative.RepresentativeKey = CASE WHEN _crmElementHierarchy.element_struct_code IN ('SALES', 'STAFF') THEN _crmElementHierarchy.element_id END AND _DimRepresentative.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimOrganisation AS _DimOrganisation ON _DimOrganisation.OrganisationKey = CASE WHEN _crmElementHierarchy.element_struct_code IN ('CLIENT', 'SALES', 'STAFF') THEN _crmElementHierarchy.parent_id WHEN _crmElementHierarchy.element_struct_code IN ('COMPANY', 'DEPARTMENT', 'GROUP', 'SALESDEPT', 'SALESGRP') THEN _crmElementHierarchy.element_id END AND _DimOrganisation.Meta_IsCurrent = 1 WHERE nc_sales_involvement.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR nc_involvement_type.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID;
