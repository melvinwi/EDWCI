...
CREATE PROCEDURE [transform].[FactUsagePricePlan_Gas]
	   ...
	   ELSE CONVERT (nchar (8) , COALESCE ( nc_product_item.date_terminated , '9999-12-31') , 112) END,
        CAST (CASE utl_account_status.accnt_status_class_id
              ...
              WHEN 4 THEN N'Error'
                  ELSE N'Closed'
              END AS nchar (10)) ,
	   CONVERT (nchar (8) , COALESCE ( utl_account_price_plan.start_date , '9999-12-31') , 112),
	   CONVERT (nchar (8) , COALESCE ( utl_account_price_plan.end_date , '9999-12-31') , 112),
	   CAST(utl_account_price_plan.acnt_price_plan_id AS nvarchar(10)) + N'.' + CAST(utl_meter.meter_id AS nvarchar(10))
          FROM DW_Staging.orion.utl_account_price_plan
								  INNER JOIN DW_Staging.orion.nc_product_item
               ON nc_product_item.seq_product_item_id = utl_account_price_plan.seq_product_item_id
              ...
              AND _DimProduct.Meta_IsCurrent = 1
               ...
               ON _DimPricePlan.PricePlanKey = N'USAGE.' + CAST(utl_account_price_plan.price_plan_id AS nvarchar(10)) + N'.G'
              AND _DimPricePlan.Meta_IsCurrent = 1
								    INNER JOIN DW_Staging.orion.utl_meter
			ON utl_meter.site_id = nc_product_item.site_id
								  INNER JOIN DW_Dimensional.DW.DimMeterRegister AS _DimMeterRegister
               ON _DimMeterRegister.MeterRegisterKey = utl_meter.meter_id
              AND _DimMeterRegister.Meta_IsCurrent = 1
								    INNER JOIN DW_Staging.orion.utl_price_plan
			 ON utl_price_plan.price_plan_id = utl_account_price_plan.price_plan_id
                                          LEFT JOIN DW_Staging.orion.utl_account_status
               ON utl_account_status.accnt_status_id = nc_product_item.accnt_status_id
          WHERE nc_product.seq_product_type_id = 3 
		AND utl_price_plan.seq_product_type_id = 3
		AND (utl_account_price_plan.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID
...
BEGIN
    ...
    --/
	   ...
	   CASE utl_account_status.accnt_status_class_id
        WHEN 1 THEN
        CONVERT (nchar (8) , COALESCE ( utl_account_price_plan.end_date, nc_product_item.date_terminated , nc_product_item.accnt_status_date, '9999-12-31') , 112) 
        WHEN 2 THEN
	   CONVERT (nchar (8) , COALESCE ( utl_account_price_plan.end_date , '9999-12-31') , 112)
