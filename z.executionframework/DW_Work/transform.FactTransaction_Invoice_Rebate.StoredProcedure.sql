...
BEGIN
    ...
    AS (SELECT utl_account_price_plan.seq_product_item_id,
          ...
          WHERE (utl_account_price_plan.end_date IS NULL
            ...
            AND utl_account_price_plan.start_date < GETDATE () 
    ), productKey
    ...
    AS (SELECT utl_pp_schedule_type.sched_type_id,
               utl_pp_schedule_type.sched_type_code,
               COALESCE (utl_pp_schedule_type.seq_account_id, ar_account.seq_account_id) AS seq_account_id,
               utl_pp_schedule_type.Meta_LatestUpdate_TaskExecutionInstanceId
          FROM
               DW_Staging.orion.utl_pp_schedule_type LEFT JOIN DW_Staging.orion.utl_pp_schedule_class
               ON utl_pp_schedule_class.sched_class_id = utl_pp_schedule_type.sched_class_id
                                                     LEFT JOIN DW_Staging.orion.nc_product_type
               ON nc_product_type.seq_product_type_id = utl_pp_schedule_type.seq_product_type_id
                                                     LEFT JOIN DW_Staging.orion.ar_account
               ON ar_account.account_code = LEFT (utl_pp_schedule_class.sched_class_code, 1) + CASE nc_product_type.product_type_code
                                                                                               WHEN 'POWER' THEN 'ELECT'
                                                                                                   ELSE nc_product_type.product_type_code
                                                                                               END
    ) 
    ...
    N'REB' + CAST (utl_transaction.trans_id AS nvarchar (11)) + N'.' + CAST (utl_transaction.trans_seq AS nvarchar (2)) ,
    -1,
...
CONVERT (nchar (8) , COALESCE ( utl_transaction.end_date , '9999-12-31') , 112)  
      FROM
          ...
          AND utl_transaction_type.trans_type_code = 'REBATE'
       ...
       ON inv_invoice_header.seq_invoice_header_id = inv_invoice_detail.seq_invoice_header_id
          ...
          AND _DimProduct.Meta_IsCurrent = 1
           ...
           ON _DimFinancialAccount.FinancialAccountKey = ppScheduleType.seq_account_id
