...
--/
	...
	INSERT INTO temp.FactMarketTransaction (
	  ...
	  SELECT
		...
		nem_transaction.nem_trans_id,
		COALESCE(NULLIF( nem_transaction.nem_init_trans_id , ''), nem_transaction.nem_trans_id ),
		CASE WHEN nem_transaction.xml_text LIKE '%<MoveInFlag>true</MoveInFlag>%' THEN 'Yes' WHEN nem_transaction.xml_text LIKE '%<MoveInFlag>false</MoveInFlag>%' THEN 'No' END,
		/* nem_transaction.trans_id */ 1
	  FROM DW_Staging.orion.nem_transaction INNER JOIN DW_Staging.orion.nem_transaction_type ON nem_transaction_type.trans_type_id = nem_transaction.trans_type_id INNER JOIN DW_Staging.orion.nem_trans_status ON nem_trans_status.trans_status_id = nem_transaction.trans_status_id LEFT JOIN DW_Staging.orion.nc_product_item ON nc_product_item.seq_product_item_id = nem_transaction.seq_product_item_id LEFT JOIN DW_Staging.orion.nc_product ON nc_product.seq_product_id = nc_product_item.seq_product_id LEFT JOIN DW_Dimensional.DW.DimAccount AS _DimAccount ON _DimAccount.AccountKey = nc_product.seq_party_id AND _DimAccount.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimService AS _DimService ON _DimService.ServiceKey = nem_transaction.site_id AND _DimService.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimChangeReason AS _DimChangeReason ON _DimChangeReason.ChangeReasonKey = nem_transaction.change_reason_id AND _DimChangeReason.Meta_IsCurrent = 1 WHERE nem_transaction.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR nem_transaction_type.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR nem_trans_status.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID;
