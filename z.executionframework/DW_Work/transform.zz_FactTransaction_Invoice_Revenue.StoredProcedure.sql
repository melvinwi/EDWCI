...
GO
CREATE PROCEDURE [transform].[zz_FactTransaction_Invoice_Revenue]
...
BEGIN
    ...
    --Insert into temporary table
	  ...
	  SELECT
		...
		COALESCE( utl_transaction.net_amount , 0) * COALESCE(utl_transaction.tax_rate, 0.10),
	  ...
	  FROM DW_Staging.orion.utl_transaction INNER JOIN DW_Staging.orion.utl_transaction_type ON utl_transaction_type.trans_type_id = utl_transaction.trans_type_id AND utl_transaction_type.trans_type_code NOT IN ('CONC', 'REBATE') INNER JOIN DW_Staging.orion.inv_invoice_detail ON inv_invoice_detail.seq_invoice_detail_id = utl_transaction.seq_invoice_detail_id INNER JOIN DW_Staging.orion.ar_invoice ON ar_invoice.seq_ar_invoice_id = inv_invoice_detail.seq_invoice_header_id INNER JOIN DW_Staging.orion.nc_client ON nc_client.seq_party_id = ar_invoice.seq_party_id INNER JOIN DW_Staging.orion.crm_element_hierarchy ON crm_element_hierarchy.element_id = nc_client.seq_party_id AND crm_element_hierarchy.element_struct_code = 'CLIENT' INNER JOIN DW_Staging.orion.crm_party_type ON crm_party_type.seq_party_type_id = crm_element_hierarchy.seq_element_type_id AND ISNULL(crm_party_type.party_type, '') <> 'SUBCLIENT' INNER JOIN DW_Staging.orion.utl_pp_schedule ON utl_pp_schedule.sched_id = utl_transaction.sched_id INNER JOIN ppScheduleType ON ppScheduleType.sched_type_id = utl_pp_schedule.sched_type_id LEFT JOIN DW_Staging.orion.utl_meter ON utl_meter.meter_id = utl_transaction.meter_id AND utl_meter.site_id = utl_transaction.site_id LEFT JOIN DW_Staging.orion.utl_pp_simple_schedule ON utl_pp_simple_schedule.pp_simple_sched_id = utl_meter.pp_simple_sched_id LEFT JOIN productKey as _productKey ON _productKey.seq_product_item_id = utl_transaction.seq_product_item_id LEFT JOIN DW_Staging.orion.utl_meter_type ON utl_meter_type.meter_type_id = utl_pp_schedule.meter_type_id LEFT JOIN DW_Staging.orion.utl_meter_class ON utl_meter_class.meter_class_id = utl_meter_type.meter_class_id  INNER JOIN DW_Dimensional.DW.DimAccount AS _DimAccount ON _DimAccount.AccountKey = nc_client.seq_party_id AND _DimAccount.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimService AS _DimService ON _DimService.ServiceKey = utl_transaction.site_id AND _DimService.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimProduct AS _DimProduct ON _DimProduct.ProductKey = _productKey.ProductKey AND _DimProduct.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimFinancialAccount AS _DimFinancialAccount ON _DimFinancialAccount.FinancialAccountKey = CASE WHEN ppScheduleType.sched_type_code IN ('USAGE', 'DEMAND') THEN COALESCE(utl_pp_simple_schedule.seq_rev_account_id, utl_pp_schedule.seq_account_id, ppScheduleType.seq_account_id) ELSE COALESCE(utl_pp_schedule.seq_account_id, ppScheduleType.seq_account_id) END AND _DimFinancialAccount.Meta_IsCurrent = 1 LEFT JOIN DW_Dimensional.DW.DimVersion AS _DimVersion ON _DimVersion.VersionKey = 'Actual' LEFT JOIN DW_Dimensional.DW.DimUnitType AS _DimUnitType ON _DimUnitType.UnitTypeKey = utl_meter_class.uom_id AND _DimUnitType.Meta_IsCurrent = 1 WHERE utl_transaction.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR utl_transaction_type.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR inv_invoice_detail.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR ar_invoice.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR utl_pp_schedule.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR ppScheduleType.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR utl_meter.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR utl_pp_simple_schedule.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR _productKey.Meta_HasChanged = 1 OR utl_meter_type.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR utl_meter_class.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID;
    --/
