...
/*
Usage:
  EXEC [dbo].[ToggleIndexes]  @DatabaseName           = '[DW_Dimensional]'
                            , @SchemaName             = '[dw]'
                            , @TableName              = '[FactTransaction]'
                            , @Operation              = 'REBUILD'  --'DISABLE' / 'REBUILD'
                            , @RebuildEnabledIndexes  = 1
                            , @Debug                  = 1
*/
...
SELECT       ''ALTER INDEX ['' + NAME + ''] ON ' + @DatabaseName + '.' + @SchemaName + '.' + @TableName + ' ' + @Operation + ' ''
              + CASE  WHEN @Operation = ''REBUILD'' AND TYPE_DESC = ''NONCLUSTERED''           THEN ''PARTITION = ALL WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = ON, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON);''
                      WHEN @Operation = ''REBUILD'' AND TYPE_DESC = ''CLUSTERED COLUMNSTORE''  THEN ''PARTITION = ALL WITH (DATA_COMPRESSION = COLUMNSTORE);'' 
                ELSE '';'' END
              , ROW_NUMBER() OVER (PARTITION BY NULL ORDER BY NAME)
FROM        ' + @DatabaseName + '.SYS.INDEXES
WHERE       OBJECT_ID = OBJECT_ID(''' + @DatabaseName + '.' + @SchemaName + '.' + @TableName + ''') 
        AND TYPE_DESC IN  ( ''NONCLUSTERED''
                          , ''CLUSTERED COLUMNSTORE''
                          )
        AND NOT (IS_DISABLED = 1 AND @Operation = ''DISABLE'')
        ' + CASE WHEN @RebuildEnabledIndexes = 0 THEN 'AND NOT (IS_DISABLED = 0 AND @Operation = ''REBUILD'')' ELSE '' END + '
ORDER BY  NAME'
