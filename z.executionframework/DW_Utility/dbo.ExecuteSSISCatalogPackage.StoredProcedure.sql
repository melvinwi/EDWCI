...
AS
 ...
 /*
  ...
  JG      14/11/2014  Added EXEC [log].[LogTaskParameterChange] to log TaskParameters
  JG      08/12/2014  Removed NVARCHAR(128) casting on [ParameterValue]
  JG      18/12/2014  Added SQL_VARIANT_PROPERTY() function to the parameter setting section, in order to cast any VARCHAR parameter values as NVARCHAR, when calling the procedure 
                      [SSISDB].[catalog].[set_execution_parameter_value] (If any further datatypes are incompatible with this procedure, they should be added to the CASE statement.)
  <YOUR ROW HERE>
...
BEGIN TRY
  ...
  SET @TaskParameter_Cursor = CURSOR FOR
      SELECT    [ParameterName]  
              ...
              , CASE WHEN SQL_VARIANT_PROPERTY ( [ParameterValue] , 'BaseType' ) = 'varchar' THEN CAST([ParameterValue] AS NVARCHAR(128)) ELSE [ParameterValue] END
              , [ObjectTypeId]
      FROM      [config].[TaskParameter] 
      WHERE     [TaskId]      =   @TaskId
        AND     [IsDisabled]  <>  1
