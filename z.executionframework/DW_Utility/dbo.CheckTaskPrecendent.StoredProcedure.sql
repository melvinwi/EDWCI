...
AS
  ...
  */
  ...
  SELECT @PrecendentComplete = MIN( CASE  WHEN  ISNULL(StatusCode, 'S') = 'S'       THEN  1   --Precendent succeeded or none exists
                                          WHEN  StatusCode IN ('A', 'F', 'U', 'P')  THEN  -1  --Precendent failed : the SSIS 'task' package won't attempt the task
                                          WHEN  StatusCode IN ('I', 'W')            THEN  0   --Precendent pending : the SSIS 'task' package will sleep and check again later
                                                                                    ELSE  0
                                    END
                                  )
  FROM  PrecedentsByPrecedence
