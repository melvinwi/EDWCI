...
--/
	...
	INSERT INTO lumo.DimService (
	  ...
	  SELECT
		...
		ISNULL( _meterDailyConsumption.est_daily_consumption ,0.0),
	  ...
	  FROM lumo.utl_site INNER JOIN lumo.utl_distrib_loss_factor_sched ON utl_distrib_loss_factor_sched.dlf_id = ISNULL(utl_site.dlf_id,4) LEFT JOIN meterDailyConsumption AS _meterDailyConsumption on _meterDailyConsumption.site_id = utl_site.site_id WHERE utl_distrib_loss_factor_sched.start_date < GETDATE() AND ISNULL(utl_distrib_loss_factor_sched.end_date,'9999-12-31') > GETDATE() AND (utl_site.Meta_LatestUpdate_TaskExecutionInstanceId  > @LatestSuccessfulTaskExecutionInstanceID OR utl_distrib_loss_factor_sched.Meta_LatestUpdate_TaskExecutionInstanceId  > @LatestSuccessfulTaskExecutionInstanceID OR _meterDailyConsumption.Meta_HasChanged = 1);
