CREATE PROCEDURE lumo.TRANSFORM_FactUsagePricePlan_Gas
...
--/
	...
	INSERT INTO lumo.FactUsagePricePlan (
	  ...
	  SELECT
		...
		CASE utl_account_status.accnt_status_class_id WHEN 1 THEN CONVERT (nchar (8) , COALESCE ( nc_product_item.date_terminated , nc_product_item.accnt_status_date, '9999-12-31') , 112) WHEN 2 THEN 99991231 ELSE CONVERT (nchar (8) , COALESCE ( nc_product_item.date_terminated , '9999-12-31') , 112) END,
		...
		CONVERT(NCHAR(8), ISNULL( utl_account_price_plan.start_date , '9999-12-31'), 112),
		CASE utl_account_status.accnt_status_class_id WHEN 1 THEN CONVERT (nchar (8) , COALESCE ( utl_account_price_plan.end_date , nc_product_item.date_terminated , nc_product_item.accnt_status_date, '9999-12-31') , 112) WHEN 2 THEN CONVERT (nchar (8) , COALESCE ( utl_account_price_plan.end_date , '9999-12-31') , 112) ELSE CONVERT (nchar (8) , COALESCE ( utl_account_price_plan.end_date, nc_product_item.date_terminated , '9999-12-31') , 112) END,
		CAST( utl_account_price_plan.acnt_price_plan_id AS nvarchar(10)) + N'.' + CAST(utl_meter.meter_id AS nvarchar(10))
	  FROM /* Staging */ lumo.utl_account_price_plan INNER JOIN /* Staging */ lumo.nc_product_item ON nc_product_item.seq_product_item_id = utl_account_price_plan.seq_product_item_id INNER JOIN /* Staging */ lumo.nc_product ON nc_product.seq_product_id = nc_product_item.seq_product_id INNER JOIN /* Dimensional */ lumo.DimAccount AS _DimAccount ON _DimAccount.AccountKey = nc_product.seq_party_id AND _DimAccount.Meta_IsCurrent = 1 INNER JOIN /* Dimensional */ lumo.DimService AS _DimService ON _DimService.ServiceKey = CAST (nc_product_item.site_id AS int) AND _DimService.Meta_IsCurrent = 1 INNER JOIN productKey AS _productKey ON _productKey.seq_product_item_id = nc_product_item.seq_product_item_id LEFT JOIN /* Dimensional */ lumo.DimProduct AS _DimProduct ON _DimProduct.ProductKey = _productKey.ProductKey AND _DimProduct.Meta_IsCurrent = 1 LEFT JOIN /* Dimensional */ lumo.DimPricePlan AS _DimPricePlan ON _DimPricePlan.PricePlanKey = N'USAGE.' + CAST(utl_account_price_plan.price_plan_id AS nvarchar(10)) + N'.G'AND _DimPricePlan.Meta_IsCurrent = 1 INNER JOIN /* Staging */ lumo.utl_meter ON utl_meter.site_id = nc_product_item.site_id INNER JOIN /* Dimensional */ lumo.DimMeterRegister AS _DimMeterRegister ON _DimMeterRegister.MeterRegisterKey = utl_meter.meter_id AND _DimMeterRegister.Meta_IsCurrent = 1 INNER JOIN /* Staging */ lumo.utl_price_plan ON utl_price_plan.price_plan_id = utl_account_price_plan.price_plan_id LEFT JOIN /* Staging */ lumo.utl_account_status ON utl_account_status.accnt_status_id = nc_product_item.accnt_status_id WHERE nc_product.seq_product_type_id = 3 AND utl_price_plan.seq_product_type_id = 3 AND (utl_account_price_plan.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR nc_product.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR nc_product_item.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR _productKey.Meta_HasChanged = 1 OR utl_account_status.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR _DimAccount.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR _DimService.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR _DimProduct.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR _DimPricePlan.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID OR _DimMeterRegister.Meta_LatestUpdate_TaskExecutionInstanceId > @LatestSuccessfulTaskExecutionInstanceID);
