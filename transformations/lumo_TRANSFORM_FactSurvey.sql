...
--/
  ...
  + '' + CAST(_CX_Monitor.RespondentId AS VARCHAR(MAX))
	  FROM lumo.CX_MonitorCodes   LEFT JOIN _CX_Monitor ON CX_MonitorCodes.QuestionKey = _CX_Monitor.QuestionId   AND CX_MonitorCodes.Value = _CX_Monitor.Data  LEFT JOIN _CX_Monitor_Cust ON _CX_Monitor.RespondentId = _CX_Monitor_Cust.RespondentId  LEFT JOIN _Question_Id ON _CX_Monitor.QuestionRef = _Question_Id.QuestionKey   WHERE 1=1  AND _CX_Monitor.QuestionId IS NOT NULL;
