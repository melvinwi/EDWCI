...
|WITH_STATEMENTS	|WITH customerStatus AS (SELECT "nc_client":../staging/nc_client.textile .seq_party_id, MAX (CASE WHEN nc_product_item.accnt_status_id = '6'OR nc_product_item.accnt_status_id = '8' THEN 1 ELSE 0 END) AS CustomerStatus, MAX (CASE WHEN "nc_client":../staging/nc_client.textile .Meta_ChangeFlag = 1 OR nc_product.Meta_ChangeFlag = 1 OR nc_product_item.Meta_ChangeFlag = 1 THEN 1 ELSE 0 END) AS Meta_ChangeFlag FROM [schema]."nc_client":../staging/nc_client.textile  LEFT OUTER JOIN [schema].nc_product ON nc_product.seq_party_id = "nc_client":../staging/nc_client.textile .seq_party_id LEFT OUTER JOIN [schema].nc_product_item ON nc_product_item.seq_product_id = nc_product.seq_product_id GROUP BY "nc_client":../staging/nc_client.textile .seq_party_id)	|	|	|	|
