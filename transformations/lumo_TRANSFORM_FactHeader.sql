...
--/
	...
	INSERT INTO lumo.FactHeader (
	  ...
	  SELECT
		...
		inv_invoice_header.invoice_amount,
		inv_invoice_header.tax_amount,
		inv_invoice_header.prompt_payment_disc,
		CASE ar_invoice.paid_promptly WHEN 'Y' THEN 'Yes' ELSE 'No' END,
		CASE ar_invoice.reversal WHEN 'Y' THEN 'Yes' ELSE 'No' END,
		inv_invoice_header.seq_invoice_run_id,
